...
| 1.3d | 后端 | Provider 健康检查（`t_provider_health`）：定时探测（每分钟）+ 每次调用 LLM 更新，记录连续失败次数、最近延迟、最后成功/失败时间、失败原因 | 1.3c |
| 1.4 | 前端 | Axios 封装完善 + API 类型定义 | ciff-web 基础 ✅ |
| 1.5 | 前端 | Provider 管理页面对接真实 API（替换 mock） | 1.4 + 1.1 |
| 1.6 | 前端 | ~~Model 管理页面，路由采用嵌套形式 `/providers/:id/models`~~ **【降级】** 改为在 Provider 列表弹窗内管理模型，降低交互跳转成本 | 1.5 + 1.2 |
...
| T1.6b | 单元 | `ProviderHealthServiceTest`：成功时重置连续失败 / 失败时累加 / 延迟记录 / 状态判定（healthy/unhealthy/unknown）/ 定时探测更新 | ciff-provider |
...
| T1.10 | 前端单元 | `ModelList.spec.ts`：模型列表弹窗 / 新增模型绑定供应商 / 参数编辑 | ciff-web |
...
>
> **设计决策**：V1 Agent 不做自主多步推理（ReAct / Function Calling 循环），多步编排是 Workflow 的职责。Agent 的工具调用为单轮——LLM 判断是否需要调工具 → 调用一次 → 拿到结果 → 回复用户。Agent 本质是 Chatbot + 可选的工具增强。
...
>
> **V1 向量维度约定**：`t_knowledge_chunk.embedding` 固定为 `VECTOR(1024)`（阿里云 text-embedding-v3）。V1 不支持其他维度的 embedding 模型，`t_knowledge.embedding_model` 配置需校验兼容性。
...
| 4.4 | 后端 | Agent 工具调用（单轮）：解析 tool_call → 执行工具 → 回传结果。不实现多步循环，多步编排由 Workflow 承担 | Phase 2.3 + Phase 1.3 |
...
| T4.6 | 单元 | `ToolCallingTest`：单轮工具调用 / 工具超时降级 / 工具异常回退 / 无工具时直接对话 | ciff-chat |
...
4. 工具调用：单轮结果正确（LLM 返回 tool_call → 执行 → 回传），工具超时/异常有降级处理，不影响对话继续
...
- [x] **Phase 0**：前置准备（UserContext + 枚举 + Flyway + 跨线程传递）
- [x] **Phase 1**：Provider 基础层（开发 + 测试） ✅ 2026-04-17
- [x] **Phase 2**：Agent 基础 + MCP 工具（开发 + 测试） ✅ 2026-04-18
- [x] **Phase 3**：知识库 + Agent 增强（开发 + 测试） ✅ 2026-04-25
- [x] **Phase 4**：Chat 对话引擎（后端 + 前端 + 测试） ✅ 2026-04-21
- [x] **Phase 5**：Workflow 工作流（开发 + 测试） ✅ 2026-04-23
- [x] **Phase 6**：API 发布 + 用户认证 + 部署（开发 + 测试） ✅ 2026-04-25（Docker 部署待续）
...
| R-4 | 向量维度硬编码 `VECTOR(1024)` 与 embedding_model 可能不匹配 | Phase 3 新增"V1 向量维度约定"：固定 1024 维（text-embedding-v3），仅支持兼容模型，embedding_model 配置需校验 |
