...
| 序号 | 端 | 任务 | 前置依赖 |
|------|-----|------|----------|
| 1.1 | 后端 | `t_provider` CRUD：Entity → Mapper → Service → Controller → Facade。**type 字段使用 ciff-common 定义的 `ProviderType` 枚举（唯一事实源）** | Phase 0 ✅ |
...
7. 单条 CRUD 接口 P95 < 200ms（本地开发环境、单线程顺序执行、数据量 < 1000 条，仅作回归参考）
...
| T3.3 | Mapper 集成 | `KnowledgeMapperTest`：CRUD + `UserContext` user_id 维度查询 | ciff-knowledge |
