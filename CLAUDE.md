...
- JSON 操作优先使用 `JsonUtil` 工具类，禁止在业务代码中直接使用 `ObjectMapper` 或 `JsonNode`
...
## Karpathy 防错规范（来自 [Karpathy 观察](https://x.com/karpathy/status/2015883857489522876)）

### 动手前先想清楚
- **显式陈述假设**：不偷偷做选择。需求有多种解读时，列出来让我拍板，不要自己悄悄选一个
- **卡壳就停**：哪里不清楚就直接说"X 不清楚"并提问，不要硬编下去
- **更简单的做法**：如果有更直接的方案，先说出来再确认，必要时反推我的需求

### 简洁优先（自检：senior 看了会不会嫌啰嗦）
- 写完问自己：200 行能用 50 行解决吗？能就重写
- 不写假设性的"灵活性 / 可配置性"——除非我点名要
- 不为不可能发生的场景写错误处理

### 外科手术式改动
- 只动需求范围内的代码——不顺手"优化"邻近代码、注释、格式
- 风格保持和文件已有风格一致，即使你觉得自己有更好的写法
- 看到无关的死代码，**告诉我**，不要自己删
- 只清理**你自己**改动造成的孤儿（unused imports / 变量 / 函数）；老代码里的死代码不动

### 目标驱动执行
- 把任务翻译成可验证目标，再动手：
  - "加校验" → "先写非法输入的测试，让它失败 → 让它过"
  - "修 bug" → "先写能复现的测试 → 让它过"
  - "重构 X" → "改前改后跑同一套测试，都过"
- 多步任务先列计划，每步带验证标准：
  ```
